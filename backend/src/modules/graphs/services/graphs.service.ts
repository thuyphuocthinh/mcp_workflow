...
export class GraphService {
  ...
  async cloneGraph(graphId: string, userId: string): Promise<SuccessResponse> {
    ...
    const populated = await newGraph.populate('user_id', 'email name');
  ...
  async getGraphRaw(graphId: string, userId: string): Promise<GraphDocument> {
    const graph = await this.graphModel
      .findOne({
        _id: new Types.ObjectId(graphId),
        user_id: new Types.ObjectId(userId),
  ...
  async streamRun(graph: GraphDocument, input: string, userId?: string) {
